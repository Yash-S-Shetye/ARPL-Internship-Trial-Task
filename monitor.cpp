...
class TopicMonitor {
    ...
    TopicMonitor(const std::string& topic, const std::string& alias, const std::string& msg_type, float target_frequency)
        : topic(topic), alias(alias), msg_count(0), frequency(0), target_frequency(target_frequency) {
    }
    ...
    std::string get_status() {
        if (frequency >= target_frequency * 0.95) {
            return "  ✅      |";
        } else if (frequency >= target_frequency * 0.9) {
            return "  ⚠️      |";
        } else {
            return "  ❌      |";
        }
...
// Declaraing the global variables
std::string status;
std::string mav_name;
float frequency;
...
int monitor_frequency() {
    ...
    if(!nh.getParam("MAV_NAME", mav_name)) {
        ROS_INFO("No parameter provided. Exiting...");
        return 1;
    }
    else {
    std::string package_path = ros::package::getPath("autonomous_robots"); // Specifying the ros package path
    std::string yaml_file_path = package_path + "/config/monitor.yaml"; // Specifying the location of yaml file

    // Load data from yaml file
    YAML::Node topicsNode = YAML::LoadFile(yaml_file_path);

    // Create a monitor for each topic
    std::vector<TopicMonitor> monitors;
    for (const auto& topicNode : topicsNode) {
        std::string topic = topicNode["topic"].as<std::string>();
        std::string alias = topicNode["alias"].as<std::string>();
        std::string msg_type = topicNode["msg_type"].as<std::string>();
    ...
    while (ros::ok()) {
        ...
        for (auto& monitor : monitors) {
            if(monitor.topic == "/" + mav_name + "/main_camera/image_raw/compressed"
            || monitor.topic == "/" + mav_name + "/cam1/color/image_raw/compressed"
            || monitor.topic == "/" + mav_name + "/cam1/infra1/image_rect_raw/compressed"
            || monitor.topic == "/" + mav_name + "/cam1/infra2/image_rect_raw/compressed") {
            monitor.subscriber = nh.subscribe(monitor.topic, 1000, &TopicMonitor::callback<sensor_msgs::CompressedImage>, &monitor);
            }

            else if(monitor.topic == "/" + mav_name + "/cam1/imu" || monitor.topic == "/" + mav_name + "/mavros/imu/data") {
                monitor.subscriber = nh.subscribe(monitor.topic, 1000, &TopicMonitor::callback<sensor_msgs::Imu>, &monitor);
            }
            else if(monitor.topic == "/" + mav_name + "/mavros/global_position/raw/fix") {
                monitor.subscriber = nh.subscribe(monitor.topic, 1000, &TopicMonitor::callback<sensor_msgs::NavSatFix>, &monitor);
            }
            else if(monitor.topic == "/" + mav_name + "/odom_transform_nodelet/odomBinworld_from_transform"
                    || monitor.topic == "/" + mav_name + "/mavros/local_position/odom") {
                monitor.subscriber = nh.subscribe(monitor.topic, 1000, &TopicMonitor::callback<nav_msgs::Odometry>, &monitor);
            }else if(monitor.topic == "/" + mav_name + "/mavros/global_position/raw/gps_vel") {
